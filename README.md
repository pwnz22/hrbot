...
- 🌐 **REST API** - доступ к данным через FastAPI
- 🎨 **Web интерфейс** - Vue.js приложение (Telegram Mini App)
...
1. **bot** - Telegram бот
2. **api** - FastAPI REST API (порт 8000)
3. **webapp** - Vue.js приложение (порт 3001)
...
- `GET /vacancies/{id}` - Конкретная вакансия
